<?php 
    ...
    $idBrand = base64_decode($idBrand);
    $sql = "SELECT * from artikli INNER JOIN brands ON artikli.idBrand = brands.idBrand WHERE artikli.idBrand = '$idBrand'";
    $sqlBrand = "SELECT brandName from brands where idBrand = '$idBrand'";
    $all_brand_names = $con->query($sqlBrand);
...
<div class="articlesswipe">
                <?php
                    while($row = mysqli_fetch_assoc($all_brand_names)){
                ?>
                <h2>Articles for <span class="fire"><?php echo $row['brandName']?></span></h2>
                <?php
                 }
                ?>
            <div class="container">
